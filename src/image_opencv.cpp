...
void playWAV() {
    isPlaying = true;
    printf("스레드재생중");
    sndPlaySoundA("data/Mask_X.wav", SND_ASYNC | SND_NODEFAULT); // 백슬래
...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    ...
    try {
        //thresh = 0.7;  // 80% 이상일 때 박스생성, 카운트
        //float accuracy = 0.80; // 80% 이상일 때 카운트
                ...
                if (dets[i].prob[j] > thresh && show) {
                    if (class_id < 0) {
                        ...
                        if (strcmp(names[j], "Human face") == 0) {
                            ...
                            // 특정 클래스의  probability 에 따라 수행할 동작을 나타내는 조건문
                            if (dets[i].prob[j] >= thresh) {  // 95퍼 이상일 때 동작                     
                                if (!isPlaying) {
                                    thread t1(playWAV);
                                    t1.detach();
                                }
        ...
        std::string str_no_mask_count(std::to_string(humanCount));
                                                                   // 웹캠일 땐 90, 110
        cv::putText(*show_img, str_mask + str_mask_count, cv::Point(10, 90), cv::FONT_HERSHEY_COMPLEX_SMALL, 2 * font_size, CV_RGB(0, 255, 0), 4 * font_size, CV_AA);
        cv::putText(*show_img, str_no_mask + str_no_mask_count, cv::Point(10, 110), cv::FONT_HERSHEY_COMPLEX_SMALL, 2 * font_size, CV_RGB(255, 0, 0), 4 * font_size, CV_AA);
        // Human face, Mask 카운트 출력 코드          
